...
4. Apply all Supabase migrations in `supabase/migrations/001_core_schema.sql` through `018_security_hardening.sql` in filename order. Do not skip 009–018; they contain the current privacy, chat, graduation-book, push and security rules.
...
7. Run `flutter clean`, `flutter pub get`, `flutter analyze` and `flutter test`. The source package was statically checked here, but the Flutter SDK and Android build toolchain are not installed in the repair environment, so an actual APK/AAB build cannot be truthfully claimed as completed here.
