...
class ZameelBookExchangeService {
  ...
  static Future<List<Map<String, dynamic>>> searchBooks(
    ...
    final safe = query.trim().replaceAll(RegExp(r'[,()]'), ' ');
    final pattern = '%$safe%';
    final rows = await db
        .from('book_listings')
        .select('*, owner:users!book_listings_owner_id_fkey(id,name,profile_image,university,college,department)')
        .eq('status', 'available')
        .or('title.ilike.$pattern,author.ilike.$pattern,subject.ilike.$pattern,description.ilike.$pattern')
        .order('created_at', ascending: false)
        .limit(limit);
    return List<Map<String, dynamic>>.from(rows).map(_toUiBook).toList();
  }
  ...
  static RealtimeChannel subscribeToBookRequestMessages(
    String requestId,
    void Function() onChange,
  ) {
    return db
        .channel('book_exchange_messages:$requestId')
        .onPostgresChanges(
          event: PostgresChangeEvent.all,
          schema: 'public',
          table: 'book_exchange_messages',
  ...
  static Future<void> removeRealtimeChannel(RealtimeChannel? channel) async {
