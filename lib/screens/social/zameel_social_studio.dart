  ...
  Future<void> _publish() async {
    final source = await showModalBottomSheet<ImageSource>(
      ...
      builder: (sheetContext) => SafeArea(child: Wrap(children: [
        ListTile(leading: const Icon(Icons.videocam_rounded), title: Text(ar ? 'تسجيل كليبس الآن' : 'Record a clip'), subtitle: Text(ar ? 'بحد أقصى 45 ثانية' : 'Up to 45 seconds'), onTap: () => Navigator.pop(sheetContext, ImageSource.camera)),
        ListTile(leading: const Icon(Icons.video_library_rounded), title: Text(ar ? 'اختيار من الهاتف' : 'Choose from phone'), onTap: () => Navigator.pop(sheetContext, ImageSource.gallery)),
      ])),
    );
    ...
    try {
      ...
      final id = await ZameelSocialService.createClipBytes(bytes: bytes, filename: file.name, caption: draft.caption, durationSeconds: 45, audience: draft.audience);
