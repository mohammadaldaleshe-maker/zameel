  ...
  Future<void> _load({bool silent = false}) async {
    try {
      ...
      unawaited(MediaCacheService.prefetch(
        visible.map((clip) => clip['video_url']?.toString() ?? ''),
        limit: 6,
      ));
    } catch (_) {
...
class _VerticalClipsViewerState extends State<_VerticalClipsViewer> {
  ...
  void _prefetchAround(int index) {
    ...
    for (final candidate in <int>[index, index + 1, index - 1, index + 2]) {
      if (candidate < 0 || candidate >= _clips.length) continue;
      final url = _clips[candidate]['video_url']?.toString() ?? '';
...
class _ClipAutoPreviewState extends State<_ClipAutoPreview> {
  ...
  Future<void> _prepare() async {
    ...
    if (!kIsWeb && cachedPath == null) {
      unawaited(MediaCacheService.prefetch(<String>[widget.url], limit: 1));
    }
