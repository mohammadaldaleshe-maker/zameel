...
class _ProfileScreenState extends State<ProfileScreen> {
        ...
        child: Column(
          ...
          children: [
            ...
            if (video != null && video.isNotEmpty)
              Padding(
                padding: const EdgeInsets.only(top: 12),
                child: ClipRRect(borderRadius: BorderRadius.circular(14), child: VideoPlayerWidget(videoUrl: video)),
              ),
            const SizedBox(height: 10),
            Row(
              children: [
    ...
    showModalBottomSheet(context: context, builder: (sheetContext) => SafeArea(child: Wrap(children: [
      ...
      if (ownsPost && post['audience'] != 'private') ListTile(
        ...
        onTap: () async {
          ...
          try {
            await Supabase.instance.client.from('posts').update({'audience': 'private'}).eq('id', post['id']);
      ...
      if (ownsPost) ListTile(leading: const Icon(Icons.delete_outline_rounded, color: Colors.red), title: Text(ar ? 'حذف المنشور' : 'Delete post'), onTap: () async { Navigator.pop(sheetContext); await Supabase.instance.client.from('posts').delete().eq('id', post['id']); _load(); }),
    ])));
  }
  ...
  Future<void> _choosePostAudience(Map<String, dynamic> post, bool ar) async {
    ...
    try {
      await Supabase.instance.client.from('posts').update({'audience': selected}).eq('id', post['id']);
      if (!mounted) return;
  ...
  Future<void> _composeMediaPost(bool ar, {required bool video}) async {
    ...
    final publish = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(title: Text(video ? (ar ? 'نشر فيديو' : 'Publish video') : (ar ? 'نشر صورة' : 'Publish photo')), content: TextField(controller: caption, maxLines: 3, decoration: InputDecoration(hintText: ar ? 'أضف وصفًا اختياريًا' : 'Add an optional caption', border: const OutlineInputBorder())), actions: [TextButton(onPressed: () => Navigator.pop(ctx,false), child: Text(ar ? 'إلغاء' : 'Cancel')), FilledButton(onPressed: () => Navigator.pop(ctx,true), child: Text(ar ? 'نشر' : 'Publish'))]));
    final text = caption.text.trim(); caption.dispose(); if(publish!=true)return;
    final me=Supabase.instance.client.auth.currentUser;if(me==null)return;
    try {
      final bytes=await file.readAsBytes();
      final ext=file.name.contains('.')?file.name.split('.').last:(video?'mp4':'jpg');
      final path='${me.id}/profile_${video?'video':'image'}_${DateTime.now().millisecondsSinceEpoch}.$ext';
      await Supabase.instance.client.storage.from('posts').uploadBinary(path,bytes);
      final url=Supabase.instance.client.storage.from('posts').getPublicUrl(path);
      await Supabase.instance.client.from('posts').insert({'user_id':me.id,'type':video?'video':'image','text_ar':text,'text_en':text,'audience':audience,(video?'video_url':'image_url'):url});
      await _load();
    } catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('${ar?'تعذر النشر':'Could not publish'}: $e')));}
  }
  ...
  Future<void> _composeProfileStory(bool ar) async {
    ...
    try{final bytes=await file.readAsBytes();final url=await ZameelSocialService.uploadMediaBytes(bytes,filename:file.name,type:type=='video'?'story_video':'story');if(url==null)throw Exception('upload_failed');await ZameelSocialService.createStory(mediaUrl:url,mediaType:type,caption:'',audience:audience);if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(ar?'تم نشر الحالة ✓':'Story published ✓')));}catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('${ar?'تعذر نشر الحالة':'Could not publish story'}: $e')));}
  }
