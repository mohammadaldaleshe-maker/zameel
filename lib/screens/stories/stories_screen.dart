...
class _StoriesWidgetState extends State<StoriesWidget> {
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  setState(() {
                    // Keep every active story; users can publish multiple stories.
  ...
  Future<void> _pickImageStory() async {
    ...
    if (image != null) {
      ...
      setState(() {
        // Keep previous stories and append the new one.
        widget.stories.insert(0, {
          'id': DateTime.now().millisecondsSinceEpoch,
          'name': isArabic ? 'أنت' : 'You',
          'time': isArabic ? 'الآن' : 'Now',
  ...
  List<Map<String, dynamic>> _viewerStories(int tappedIndex) {
    if (tappedIndex < 0 || tappedIndex >= widget.stories.length) return const [];
    final tapped = widget.stories[tappedIndex];
    final ownerKey = (tapped['userId'] ?? tapped['user_id'] ?? tapped['name'] ?? tapped['name_ar'] ?? '').toString();
    final grouped = widget.stories.where((story) {
      final key = (story['userId'] ?? story['user_id'] ?? story['name'] ?? story['name_ar'] ?? '').toString();
      return key == ownerKey;
    }).toList();
    return grouped.isEmpty ? [tapped] : grouped.reversed.toList();
  }
                ...
                return _StoryCard(
                  ...
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) {
                          final viewerStories = _viewerStories(storyIndex);
                          return StoryViewScreen(
                          stories: viewerStories,
                          initialIndex: 0,
                          onStoryViewed: (index) {
                            if (index < viewerStories.length) {
                              viewerStories[index]['viewed'] = true;
                            }
                          },
                        );
                        },
