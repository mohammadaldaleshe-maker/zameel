  ...
  Future<void> _loadStories() async {
    ...
    try {
      ...
      unawaited(MediaCacheService.prefetch(
        normalized.expand((story) sync* {
          final image = story['imagePath']?.toString() ?? '';
          final video = story['videoPath']?.toString() ?? '';
          if (video.isNotEmpty) yield video;
          if (image.isNotEmpty) yield image;
        }),
        limit: 8,
      ));
    } catch (_) {
      ...
      return FutureBuilder<String?>(
        ...
        builder: (_, snapshot) {
          ...
          if (!kIsWeb && local != null) {
            return Image.file(File(local), fit: BoxFit.cover, width: 58, height: 58);
          }
  ...
  Future<void> _initialize() async {
    try {
      ...
      if (!kIsWeb && isNetworkSource) {
        ...
        if (cachedPath == null) {
          unawaited(MediaCacheService.prefetch(<String>[path], limit: 1));
        }
...
class _StoryViewScreenState extends State<StoryViewScreen> {
  ...
  void _prefetchAround(int index) {
    final urls = <String>[];
    for (final candidate in <int>[index, index + 1, index - 1, index + 2]) {
      if (candidate < 0 || candidate >= widget.stories.length) continue;
      final story = widget.stories[candidate];
    ...
    unawaited(MediaCacheService.prefetch(urls, limit: 4));
  }
