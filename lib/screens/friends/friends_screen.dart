...
class _FriendsScreenState extends State<FriendsScreen> with SingleTickerProviderStateMixin {
  ...
  bool _matches(Map<String,dynamic> u){final q=_query.trim().toLowerCase();if(q.isEmpty)return true;return ['name','username','university','college','department'].any((k)=>u[k]?.toString().toLowerCase().contains(q)==true);}
...
class _FriendSearchDelegate extends SearchDelegate<String?>{
 ...
 @override String? get searchFieldLabel=>ar?'الاسم أو الجامعة أو الكلية أو التخصص':'Name, university, college or major';
 ...
 Widget _view(BuildContext c){final q=query.trim().toLowerCase();final rows=users.where((u)=>['name','username','university','college','department'].any((k)=>u[k]?.toString().toLowerCase().contains(q)==true)).toList();return ListView(children:[for(final u in rows)ListTile(leading:const CircleAvatar(child:Icon(Icons.person)),title:Text(u['name']?.toString()??'User'),subtitle:Text('${u['university']??''} • ${u['department']??''}'),onTap:()=>close(c,query))]);}
}
