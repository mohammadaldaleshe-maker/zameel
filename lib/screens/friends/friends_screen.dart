...
class _FriendsScreenState extends State<FriendsScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    final initial = widget.initialTab < 0 ? 0 : (widget.initialTab > 2 ? 2 : widget.initialTab);
    _tabs = TabController(length: 3, initialIndex: initial, vsync: this);
    _load();
  ...
  void _subscribeFriendRequests() {
    final me = uid;
    if (me == null) return;
    _requestsChannel = db.channel('zameel-friend-requests:$me');
    _requestsChannel!
        .onPostgresChanges(
  ...
  Future<void> _load({bool showSpinner = true}) async {
    ...
    try {
      ...
      final suggestedRows = await ColleagueSuggestionService.suggestions();
      ...
      final suggestions = suggestedRows
          .where((u) => !existing.contains((u['user_id'] ?? u['id'])?.toString()))
          .map((u) => {
                ...u,
  ...
  Future<void> _sendRequest(String targetId) async {
    ...
    try {
      await ColleagueSuggestionService.sendRequest(targetId);
      await _load();
      if (mounted) {
  ...
  Future<void> _respond(Map<String, dynamic> request, bool accept) async {
    ...
    try {
      ...
      // The database trigger creates the accepted/rejected notification.
      // Keeping this client free of notification inserts prevents duplicates.
      await _load();
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(textDirection: ar ? ui.TextDirection.rtl : ui.TextDirection.ltr, child: Scaffold(
      ...
      appBar: AppBar(title: Text(ar ? '👥 الزملاء' : '👥 Colleagues'), centerTitle: true, actions: [IconButton(tooltip:ar?'بحث':'Search',onPressed:()=>_showSearch(ar),icon:const Icon(Icons.search_rounded)),IconButton(onPressed: _load, icon: const Icon(Icons.refresh_rounded))], bottom: TabBar(controller: _tabs, tabs: [Tab(text: ar ? 'زملائي (${_friends.length})' : 'Colleagues (${_friends.length})'), Tab(text: ar ? 'الطلبات (${_incoming.length})' : 'Requests (${_incoming.length})'), Tab(text: ar ? 'اكتشاف' : 'Discover')])),
  ...
  Future<void> _showSearch(bool ar) async {
    final value=await showSearch<String?>(context:context,delegate:_FriendSearchDelegate([..._friends,..._suggestions],ar));
    if(value!=null&&mounted)setState(()=>_query=value);
  }

  bool _matches(Map<String,dynamic> u){final q=_query.trim().toLowerCase();if(q.isEmpty)return true;return ['name','username','university','college','department','match_reason'].any((k)=>u[k]?.toString().toLowerCase().contains(q)==true);}
  ...
  Widget _discoverView(bool ar) { final shown=_suggestions.where(_matches).toList();if (shown.isEmpty) return _empty(ar ? 'لا يوجد زملاء مطابقون' : 'No matching suggestions'); return ListView.builder(padding: const EdgeInsets.all(14), itemCount: shown.length, itemBuilder: (_, i) { final u = shown[i]; final reason=(u['match_reason']?.toString().trim().isNotEmpty==true)?u['match_reason'].toString():(ar?'اقتراح مناسب لك':'Suggested for you'); return Card(child: ListTile(leading: _avatar(u), title: Text(u['name']?.toString() ?? 'User', style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(reason), trailing: FilledButton.icon(onPressed: () => _sendRequest(u['id'].toString()), icon: const Icon(Icons.person_add_alt_1_rounded, size: 18), label: Text(ar ? 'إضافة' : 'Add')))); }); }

  Widget _empty(String text) => Center(child: Padding(padding: const EdgeInsets.all(30), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.people_outline_rounded, size: 64, color: AppTheme.muted.shade400), const SizedBox(height: 12), Text(text, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700), textAlign: TextAlign.center)])));
...
class _FriendSearchDelegate extends SearchDelegate<String?>{
 final List<Map<String,dynamic>> users;final bool ar;_FriendSearchDelegate(this.users,this.ar);
 @override String? get searchFieldLabel=>ar?'ابحث في الزملاء المقترحين':'Search suggested colleagues';
 ...
 @override Widget buildResults(BuildContext c)=>_view(c);@override Widget buildSuggestions(BuildContext c)=>_view(c);
 Widget _view(BuildContext c){final q=query.trim().toLowerCase();final rows=users.where((u)=>['name','username','university','college','department','match_reason'].any((k)=>u[k]?.toString().toLowerCase().contains(q)==true)).toList();return ListView(children:[for(final u in rows)ListTile(leading:const CircleAvatar(child:Icon(Icons.person)),title:Text(u['name']?.toString()??'User'),subtitle:Text((u['match_reason']?.toString().trim().isNotEmpty==true)?u['match_reason'].toString():'${u['university']??''} • ${u['department']??''}'),onTap:()=>close(c,query))]);}
}
