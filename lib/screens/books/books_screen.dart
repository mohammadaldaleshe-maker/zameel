...
class _BooksScreenState extends State<BooksScreen> {
  ...
  Future<void> _showIncomingRequests() async {
    ...
    try {
      final requests = await ZameelBookExchangeService.incomingRequests();
      if (!mounted) return;
          ...
          builder: (context, setSheetState) => SafeArea(
            child: SizedBox(
              ...
              child: requests.isEmpty
                  ? Center(child: Text(ar ? 'لا توجد طلبات كتب' : 'No book requests'))
                  : ListView.builder(
                      padding: const EdgeInsets.all(12),
                      itemCount: requests.length,
                      itemBuilder: (_, index) {
                        final request = requests[index];
                        final pending = request['request_status'] == 'pending';
                        final offered = request['offered_title']?.toString();
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text([
                              ...
                              if (offered != null && offered.isNotEmpty)
                                ar ? 'مقابل: $offered' : 'Offers: $offered',
  ...
  List<Map<String, dynamic>> get filteredBooks {
    ...
    return books.where((book) {
      ...
      final matchesAcademicScope = _myUniversity.isNotEmpty && _myCollege.isNotEmpty &&
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              tooltip: isArabic ? 'طلبات الكتب' : 'Book requests',
            ...
            if (!showStudyFiles) ...[
              Padding(
                ...
                child: Align(
                  ...
                  child: Text(
                    isArabic ? '📚 كتب كليتك فقط: $_myCollege' : '📚 Books from your college only: $_myCollege',
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 12),
                            Text(isArabic ? 'لا توجد كتب' : 'No books found', style: const TextStyle(fontSize: 18, color: AppTheme.muted, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 6),
                            Text(isArabic ? 'أضف كتاباً جديداً لتبدأ المبادلة' : 'Add a new book to start exchanging', style: const TextStyle(color: AppTheme.muted, fontSize: 14)),
                          ],
                        ),
                      )
                    : ListView.builder(
                              ...
                              child: AlertDialog(
                                ...
                                actions: [
                                  ...
                                  ElevatedButton(
                                    onPressed: () async {
                                      try {
                                        await ZameelBookExchangeService.requestBook(book);
