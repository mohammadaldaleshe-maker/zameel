...
class _BookImage {
  /// Stable database reference. For private Storage this remains the original
  /// object reference/public-style URL; [displayUrl] is short-lived.
  String url;
  String? displayUrl;
  double x;
  double y;
  ...
  String get viewUrl => (displayUrl?.isNotEmpty == true) ? displayUrl! : url;
...
class _GraduationBookScreenState extends State<GraduationBookScreen> {
  ...
  Future<void> _addImage(_BookPage page, {VoidCallback? onChanged}) async {
    ...
    try {
      ...
      final storageRef = storage.getPublicUrl(path);
      final displayUrl = await SecureMediaService.resolve(storageRef);
      page.images.add(_BookImage(url: storageRef, displayUrl: displayUrl));
      if (onChanged != null) {
        onChanged();
      } else if (mounted) {
        setState(() {});
                ...
                if (editable) {
                  return _BookImageEditor(
                    ...
                    onDelete: () {
                      ...
                      SecureMediaService.removeReference(image.url);
                      onChanged?.call();
                      if (onChanged == null && mounted) setState(() {});
                    },
                  );
  ...
  Future<void> _clearPage(_BookPage page) async {
    ...
    try {
      ...
      page.images.clear();
      page.elements.clear();
      for (final mediaRef in mediaToDelete) {
        await SecureMediaService.removeReference(mediaRef);
      }
      if (mounted) setState(() {});
      if (mounted) {
            ...
            Transform.rotate(
              ...
              child: Container(
                ...
                child: ClipRRect(
                  ...
                  child: Image.network(widget.image.viewUrl, fit: BoxFit.cover),
                ),
              ),
            ),
            Positioned(
