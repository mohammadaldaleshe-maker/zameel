...
class _CampusScreenState extends State<CampusScreen> {
  ...
  static const List<LatLng> _campusCoordinates = [
    LatLng(32.0170, 35.8709), LatLng(32.0159, 35.8730), LatLng(32.0165, 35.8739), LatLng(32.0152, 35.8717),
    LatLng(32.0170, 35.8734), LatLng(32.0149, 35.8730), LatLng(32.0180, 35.8720), LatLng(32.0175, 35.8713),
  ];

  void _focusBuilding(Map<String, dynamic> building) {
    final index = buildings.indexOf(building);
    if (index < 0 || index >= _campusCoordinates.length) return;
    _mapController.move(_campusCoordinates[index], 17.5);
  }
  ...
  Widget _buildMapTab(bool isArabic) {
    // University of Jordan / Amman as the default campus center.
    const center = LatLng(32.0162, 35.8719);
    final points = List.generate(buildings.length, (i) => {'building': buildings[i], 'point': _campusCoordinates[i]});
                ...
                children: [
                  ...
                  MarkerLayer(
                    markers: points.map((item) {
                      final building = item['building'] as Map<String, dynamic>;
                      ...
                      return Marker(
                        ...
                        child: GestureDetector(
                          onTap: () => _showBuildingDetails(context, building, isArabic),
                          child: Column(
                            ...
                            children: [
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: Color(building['color'] as int),
                                  shape: BoxShape.circle,
                                  boxShadow: const [BoxShadow(blurRadius: 8, color: Colors.black26)],
                                ),
                                child: Icon(building['icon'] as IconData, color: Colors.white, size: 22),
                              ),
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () => _showCampusSearch(),
                  icon: const Icon(Icons.search_rounded),
                  label: Text(isArabic ? 'البحث عن مبنى أو خدمة' : 'Find a building or service'),
                ),
              ),
              const SizedBox(width: 8),
              FilledButton.icon(
                onPressed: () => _showBuildingDetails(context, buildings[0], isArabic),
                icon: const Icon(Icons.my_location_rounded),
                label: Text(isArabic ? 'الموقع' : 'Campus'),
              ),
            ],
          ),
        ],
            ...
            actions: [
              ...
              ElevatedButton.icon(
                onPressed: () {
                  Navigator.pop(context);
                  setState(() => selectedTab = 0);
                  WidgetsBinding.instance.addPostFrameCallback((_) => _focusBuilding(building));
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        isArabic
                            ? '📍 تم تحديد ${building['name_ar']} على الخريطة'
                            : '📍 ${building['name_en']} selected on the map',
                      ),
                    ),
                  );
                },
                icon: const Icon(Icons.navigation_rounded),
                label: Text(isArabic ? 'توجيه' : 'Navigate'),
