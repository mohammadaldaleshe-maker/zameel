...
class _LiveMapScreenState extends State<LiveMapScreen>
  ...
  void _clearRouteState({bool clearDestination = false}) {
    _routeRequestId++;
    _measure.stop(canceled: true);
    _measuring = false;
    _measureFrom = null;
    _measureTo = null;
    _routePoints = const [];
  ...
  void _clearDestinationAndRoute() {
    if (!mounted) return;
    setState(() => _clearRouteState(clearDestination: true));
  }

  Future<void> _rebuildCurrentRoute() async {
    if (_destinationPoint == null) return;
    final requestId = _routeRequestId + 1;
    if (mounted) {
      setState(() {
        _clearRouteState();
      });
    ...
    _measure.addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        ...
        Future<void>.delayed(const Duration(milliseconds: 250), () {
          if (mounted && requestId == _routeRequestId && _measuring) {
            _measure.reverse();
          }
        });
  ...
  Future<void> _openCampus(_CampusData campus) async {
    _campus = campus;
    _world = false;
    _clearRouteState(clearDestination: true);
    try {
      final rows = await _db
  ...
  Future<void> _searchCampus(bool ar) async {
    ...
    await _selectDestination(selected.point, place: selected);
  }
  ...
  Future<void> _selectDestination(LatLng point, {_CampusPlace? place}) async {
    ...
    _measure.duration = Duration(
        milliseconds: (650 + math.log(math.max(1, straightMeters)) * 120)
            ...
            .toInt());
    try {
      await _measure.forward(from: 0).orCancel;
    } catch (_) {
      return;
    }
  ...
  Future<void> _buildRoute(int requestId) async {
    ...
    List<LatLng> nextPoints;
    double nextMeters;
    double nextSeconds;
    try {
      final profile = mode == 'driving' ? 'driving' : 'foot';
      ...
      nextPoints = coords.map((p) {
        final v = p as List;
        return LatLng((v[1] as num).toDouble(), (v[0] as num).toDouble());
      }).toList();
      nextMeters = (route['distance'] as num).toDouble();
      if (mode == 'walking') {
        // The public OSRM demo is primarily a driving router and may return a
        // car-like duration even when a foot profile is requested. Keep its
        // geometry/distance when available, but calculate walking time using
        // a realistic university walking pace of about 4.8 km/h.
        nextSeconds = nextMeters / 1.33;
      } else {
        final serverSeconds = (route['duration'] as num?)?.toDouble() ?? 0;
        // Inside/around a campus, avoid implausibly optimistic car times when
        // the public router returns incomplete timing data.
        final campusFallback = nextMeters / 8.33; // about 30 km/h
        nextSeconds = serverSeconds > 0
            ? math.max(serverSeconds, campusFallback * .65).toDouble()
            : campusFallback;
      }
    } catch (_) {
      nextMeters = const Distance().distance(origin, target);
      nextSeconds = nextMeters / (mode == 'driving' ? 8.33 : 1.33);
      nextPoints = [origin, target];
    }

    if (!mounted || requestId != _routeRequestId) return;
    setState(() {
      _routePoints = nextPoints;
      _routeMeters = nextMeters;
      _routeSeconds = nextSeconds;
      _routing = false;
    });
  }
