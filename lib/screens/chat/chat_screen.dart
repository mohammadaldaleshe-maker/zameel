...
class ChatDetailScreen extends StatefulWidget {
  ...
  final String? bookOfferType;
  final String? bookMemberRole;
...
class _ChatDetailScreenState extends State<ChatDetailScreen> {
  ...
  bool _pendingMatchesPersisted(
    ...
    return pendingAt.difference(persistedAt).abs() <=
  ...
  void _mergePersistedMessage(Map<String, dynamic> row) {
    ...
    setState(() {
      if (savedIndex >= 0) {
        ...
        if (pendingIndex >= 0 && pendingIndex != savedIndex) {
          _messages.removeAt(pendingIndex);
        }
      } else if (pendingIndex >= 0) {
        _messages[pendingIndex] = row;
      } else {
        _messages.add(row);
      }
    });
  ...
  Future<void> _reconcilePersistedMessages() async {
    final persisted = await _fetchPersistedMessages();
    if (!mounted) return;
  ...
  Future<void> _meetAction(bool ar) async {
    ...
    if (meet['status'] == 'pending' &&
        meet['requester_id']?.toString() == uid) {
      final createdAt = DateTime.tryParse(meet['created_at']?.toString() ?? '');
      ...
      if (elapsed < const Duration(seconds: 15)) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(ar
                  ? 'يمكنك إرسال طلب جديد بعد $remaining ثانية'
                  : 'You can send another request in $remaining seconds'),
            ),
          );
        }
        return;
  ...
  String _bookCompletionLabel(bool ar) {
    final type = widget.bookOfferType ?? '';
    final role = widget.bookMemberRole ?? '';
    if (type == 'donate') {
      return role == 'owner'
          ? (ar ? 'تم تسليم الكتاب المتبرع به' : 'Donation handed over')
          : (ar ? 'تم استلام الكتاب المتبرع به' : 'Donation received');
    }
    if (type == 'lend') {
      return role == 'owner'
          ? (ar ? 'تم تسليم الكتاب للإعارة' : 'Loan handed over')
          : (ar ? 'تم استلام الكتاب المستعار' : 'Borrowed book received');
    }
    if (type == 'sale') {
      return role == 'owner'
          ? (ar ? 'تم تسليم الكتاب للمشتري' : 'Book handed to buyer')
          : (ar ? 'تم استلام الكتاب من البائع' : 'Book received from seller');
    }
    return ar ? 'تمت عملية الكتاب' : 'Book exchange completed';
  }

  Future<void> _completeBookHandover(bool ar) async {
    final requestId = widget.bookRequestId;
    if (requestId == null) return;
    final completionLabel = _bookCompletionLabel(ar);
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: Text(ar ? 'تأكيد العملية؟' : 'Confirm completion?'),
        content: Text(
          ar
              ? '$completionLabel. سيتم تسجيل العملية وإزالة الكتاب من العروض المتاحة، مع بقاء سجل المحادثة.'
...
class _MeetColleaguePanelState extends State<MeetColleaguePanel> {
  ...
  Future<void> _sync() async {
    ...
    try {
      ...
      // Direct table reads expose the primary key as `id`, while the RPC
      // exposes it as `request_id`. Keep one stable key for later location
      // updates/end-session calls after this refresh.
      next['request_id'] ??= next['id'];
      final requesterLat = (next['requester_lat'] as num?)?.toDouble();
      final requesterLng = (next['requester_lng'] as num?)?.toDouble();
      final recipientLat = (next['recipient_lat'] as num?)?.toDouble();
      final recipientLng = (next['recipient_lng'] as num?)?.toDouble();
      final requesterAt = DateTime.tryParse(
        next['requester_updated_at']?.toString() ?? '',
      );
      final recipientAt = DateTime.tryParse(
        next['recipient_updated_at']?.toString() ?? '',
      );
      final now = DateTime.now().toUtc();
      final fresh = requesterAt != null &&
          recipientAt != null &&
          now.difference(requesterAt.toUtc()).abs() <=
              const Duration(seconds: 30) &&
          now.difference(recipientAt.toUtc()).abs() <=
              const Duration(seconds: 30);
      if (requesterLat != null &&
          requesterLng != null &&
          recipientLat != null &&
          recipientLng != null &&
          fresh) {
        final meters = const Distance().as(
          LengthUnit.Meter,
          LatLng(requesterLat, requesterLng),
          LatLng(recipientLat, recipientLng),
        );
        if (meters <= 20) {
          await Supabase.instance.client.rpc('end_meet_colleague', params: {
            'target_request_id': next['id'] ?? next['request_id'],
            'complete_meet': true,
          });
          widget.onEnded();
          return;
      ...
      if (mounted) setState(() => _meet = next);
    } catch (_) {
    } finally {
      _updating = false;
    }
