...
class _BusinessScreenState extends State<BusinessScreen> {
  ...
  final List<Map<String, dynamic>> partners = const [
    ...
    {'name': 'Campus Café', 'cat': 1, 'icon': Icons.local_cafe_rounded, 'tag': 'خصم طلابي', 'desc': 'عروض ومزايا خاصة لمستخدمي Zameel.'},
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(title: Text(ar ? 'شركاء Zameel' : 'Zameel Partners'), actions: [IconButton(onPressed: () => _showInfo(ar), icon: const Icon(Icons.info_outline_rounded))]),
        body: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              const SizedBox(height: 14),
              Row(children: [Text(ar ? 'شركاء مختارون' : 'Featured partners', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w800)), const Spacer(), Text('${filtered.length}', style: const TextStyle(color: AppTheme.muted))]),
  ...
  Widget _hero(bool ar) => Container(
    ...
    decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppTheme.primaryDark, AppTheme.primaryDark]), borderRadius: BorderRadius.circular(24)),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Container(width: 52, height: 52, decoration: BoxDecoration(color: Colors.white.withAlpha(35), borderRadius: BorderRadius.circular(16)), child: Image.asset('assets/branding/zameel_mark.png', fit: BoxFit.contain)), const SizedBox(width: 12), Expanded(child: Text(ar ? 'منظومة شركاء Zameel' : 'Zameel Partner Network', style: const TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.w800)))]),
      const SizedBox(height: 12),
  ...
  Widget _partnerCard(Map<String, dynamic> p, bool ar) => Card(margin: const EdgeInsets.only(bottom: 10), child: InkWell(onTap: () => _showPartner(p, ar), borderRadius: BorderRadius.circular(18), child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [Container(width: 54, height: 54, decoration: BoxDecoration(color: AppTheme.accentSoft, borderRadius: BorderRadius.circular(16)), child: Icon(p['icon'], color: AppTheme.primaryDark, size: 28)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Expanded(child: Text(p['name'], style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16))), Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: BoxDecoration(color: AppTheme.accentSoft, borderRadius: BorderRadius.circular(8)), child: Text(p['tag'], style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppTheme.primaryDark)))]), const SizedBox(height: 4), Text(p['desc'], maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppTheme.muted, height: 1.35))])), const Icon(Icons.chevron_left_rounded, color: AppTheme.muted)]))));
