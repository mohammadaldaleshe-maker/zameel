...
class _JobsScreenState extends State<JobsScreen> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => _searchRemote());
  }

  List<Map<String, dynamic>> get filteredJobs {
    final source = <Map<String, dynamic>>[..._remoteJobs, ...jobs];
    ...
    if (selectedFilter == 0 || _remoteJobs.isNotEmpty) return queried;
  ...
  Future<void> _searchRemote() async {
    ...
    try {
      final result = await JobsService.search(
        query: _searchController.text.trim().isEmpty ? 'student jobs' : _searchController.text.trim(),
        location: 'Jordan',
        results: 20,
      );
...
class _JobCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Color color = Color(job['color'] is int ? job['color'] as int : 0xFF18D4C6);
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    final remoteUrl = job['url']?.toString();
