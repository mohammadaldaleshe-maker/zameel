...
class _AIScreenState extends State<AIScreen> {
  ...
  final List<_AIMessage> _messages = const [
    _AIMessage(
      text:
          'مرحباً! أنا زميل AI. اسألني عن الدراسة، الجامعة أو الحياة المهنية.',
      isUser: false,
    ),
  ].toList();
  ...
  Future<void> _loadSettings() async {
    final values = await Future.wait([
      _service.memoryEnabled(),
      _service.canManageKnowledge(),
    ]);
    if (!mounted) return;
    setState(() {
      _memory = values[0];
    ...
    setState(() {
      ...
      _messages
        ...
        ..add(const _AIMessage(
          text: 'تم مسح المحادثات. كيف يمكنني مساعدتك؟',
          isUser: false,
        ));
    });
  }
