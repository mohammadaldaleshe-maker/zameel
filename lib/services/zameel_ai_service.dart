...
class ZameelAIService {
  ...
  static const chatRetention = Duration(hours: 36);
  ...
  Future<ZameelAIResult> ask({
    ...
    return ZameelAIResult(
      ...
      verified: raw['source_type'] == 'verified_faq' ||
          raw['source_type'] == 'zameel_identity',
  ...
  Future<ZameelAIChatState> loadActiveChatState() async {
    final userId = _client.auth.currentUser?.id;
    if (userId == null) {
      return const ZameelAIChatState(
        conversationId: null,
        messages: [],
        university: '',
        college: '',
    ...
    if (conversations.isNotEmpty) {
      final first = Map<String, dynamic>.from(conversations.first as Map);
      conversationId = first['id']?.toString();
