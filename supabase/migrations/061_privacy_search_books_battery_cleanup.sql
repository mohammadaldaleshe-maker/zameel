...
-- Deployed databases may use viewer_id, user_id, or both. Build the policies
-- against the physical identity columns that actually exist so this migration
-- remains safe for both fresh and legacy schemas.
...
begin
  ...
  if has_viewer_id and has_user_id then
    identity_expr := '(coalesce(viewer_id, user_id) = auth.uid())';
  elsif has_viewer_id then
    identity_expr := '(viewer_id = auth.uid())';
  elsif has_user_id then
    identity_expr := '(user_id = auth.uid())';
  else
    raise exception 'story_views has no viewer identity column';
  end if;

  execute format(
    'create policy story_views_self_write on public.story_views for insert to authenticated with check (%s and public.can_view_story(auth.uid(), story_id))',
    identity_expr
  );
  execute format(
    'create policy story_views_self_update on public.story_views for update to authenticated using (%s) with check (%s and public.can_view_story(auth.uid(), story_id))',
